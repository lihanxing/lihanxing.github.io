...
![深度图优化部分框架](/img/openMVS/深度图优化部分.png)
深度图优化是openMVS框架中，在进行完SGM/PatchMatch后的内容，执行深度图优化的原因是我们计算出的深度图或多或少会带有一些孔洞或者连通域，深度图优化为的就是对着一部分做优化。
openMVS的深度图优化有两部分：**去除连通域**和**补洞**，对应的函数分别为**RemoveSmallSegments**和**FilterDepthMap**，如上图所示，需要主要的是**去除连通域**操作在**补洞**操作的前面。

---

### 去除连通域

![](/img/openMVS/深度图优化示意图.png)
